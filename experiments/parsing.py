...
def eval(alg, data, network, pred_writer, gold_writer, punct_set, word_alphabet, pos_alphabet, device, beam=1, batch_size=256):
    ...
    for data in iterate_data(data, batch_size):
        ...
        if alg == 'graph':
            ...
            heads_pred, types_pred = network.decode(bert_words, sub_word_idx, words, chars, postags, mask=masks, leading_symbolic=conllx_data.NUM_SYMBOLIC_TAGS)
        else:
        ...
        with torch.autograd.set_detect_anomaly(True):
            for step, data in enumerate(iterate_data(data_train, batch_size, bucketed=True, unk_replace=unk_replace, shuffle=True)):
                ...
                if alg == 'graph':
                    ...
                    BERT = True
                    if BERT:
                        loss_arc, loss_type = network.loss(bert_words, sub_word_idx, words, chars, postags, heads, types, mask=masks)
                    else:
...
def parse(args):
    ...
    with torch.no_grad():
        print('Parsing...')
        start_time = time.time()
        eval(alg, data_test, network, pred_writer, gold_writer, punct_set, word_alphabet, pos_alphabet, device, beam, batch_size=args.test_batch_size)
...
if __name__ == '__main__':
    ...
    args_parser.add_argument('--test_batch_size', type=int, default=16, help='Number of sentences in each batch')
