...
def read_bucketed_data(source_path: str, word_alphabet: Alphabet, char_alphabet: Alphabet, pos_alphabet: Alphabet, type_alphabet: Alphabet,
    ...
    while inst is not None and (not max_size or counter < max_size):
        ...
        for bucket_id, bucket_size in enumerate(_buckets):
            if inst_size < bucket_size:
                data[bucket_id].append([sent.bert_ids, sent.sub_idx, sent.word_ids, sent.char_id_seqs, inst.pos_ids, inst.heads, inst.type_ids])
                max_len = max([len(char_seq) for char_seq in sent.char_seqs])
    ...
    for bucket_id in range(len(_buckets)):
        ...
        bert_wid_inputs = np.empty([bucket_size, bucket_length], dtype=np.int64)
        subword_word_indicator = np.empty([bucket_size, bucket_length], dtype=np.int64)
        ...
        bert_lengths = np.empty(bucket_size, dtype=np.int64)

        for i, inst in enumerate(data[bucket_id]):
            bert_wids, subword_ids, wids, cid_seqs, pids, hids, tids = inst
            inst_size = len(wids)
            bpe_st_size = len(bert_wids)
            sub_word_size = len(subword_ids)
            assert(inst_size==sub_word_size)
            lengths[i] = inst_size
            bert_lengths[i] = bpe_st_size
            # word ids
            wid_inputs[i, :inst_size] = wids
            ...
            bert_wid_inputs[i, bpe_st_size:] = 0
            # subword
            subword_word_indicator[i, :sub_word_size] = subword_ids
            subword_word_indicator[i, sub_word_size:] = 0
            for c, cids in enumerate(cid_seqs):
                cid_inputs[i, c, :len(cids)] = cids
                cid_inputs[i, c, len(cids):] = PAD_ID_CHAR
            cid_inputs[i, inst_size:, :] = PAD_ID_CHAR
