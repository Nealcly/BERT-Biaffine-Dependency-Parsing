...
def read_data(source_path, word_alphabet, char_alphabet, pos_alphabet, type_alphabet,
    ...
    while inst is not None and (not max_size or counter < max_size):
        ...
        data.append([sent.bert_sent_token_ids, sent.one_subword_word_indicator_ids, sent.word_ids, sent.char_id_seqs, inst.pos_ids, inst.heads, inst.type_ids, stacked_heads, children, siblings, stacked_types, skip_connect])
    ...
    for i, inst in enumerate(data):
        bert_wids, subword_ids, wids, cid_seqs, pids, hids, tids, stack_hids, chids, ssids, stack_tids, skip_ids = inst
        inst_size = len(wids)
    ...
    bert_wids = torch.from_numpy(bert_wids)
    ...
    # subword_idx = torch.from_numpy(subword_ids)
