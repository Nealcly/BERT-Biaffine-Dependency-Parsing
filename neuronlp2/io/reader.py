...
class CoNLLXReader(object):
    ...
    def getNext(self, normalize_digits=True, symbolic_root=False, symbolic_end=False):
        ...
        bert_sent_token_ids = self.tokenizer.convert_tokens_to_ids(['[CLS]'] + bert_sent_token + ['[SEP]'])


        return DependencyInstance(Sentence(bert_sent_token_ids, one_subword_word_indicator_ids, words, word_ids, char_seqs, char_id_seqs), postags, pos_ids, heads, types, type_ids)
