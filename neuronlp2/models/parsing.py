...
class DeepBiAffine(nn.Module):
    def __init__(self, word_dim, num_words, char_dim, num_chars, pos_dim, num_pos, rnn_mode, hidden_size, num_layers, num_labels, arc_space, type_space,
        ...
        self.bertlinear = nn.Linear(768, word_dim)
    ...
    def _get_rnn_output(self, input_bert_word, input_subword_idx, input_word, input_char, input_pos, mask=None):
        # [batch, length, word_dim]

        bert_output = self.bert_model(input_bert_word)[0]
        batch_size = bert_output.shape[0]
        batch_word_mask_tensor_list = []
        for idx in range(batch_size):
            one_sentence_vector = torch.index_select(bert_output[idx], 0, input_subword_idx[idx]).unsqueeze(0)
            batch_word_mask_tensor_list.append(one_sentence_vector)
        batch_word_mask_tensor = torch.cat(batch_word_mask_tensor_list, 0)
        # mask padding
        # batch_word_mask_tensor = batch_word_mask_tensor * mask.unsqueeze(2)
        # word = self.word_embed(input_word)

        word = self.bertlinear(batch_word_mask_tensor)
    ...
    def forward(self, input_bert_word, input_subword_idx, input_word, input_char, input_pos, mask=None):
        # output from rnn [batch, length, dim]
        arc, type = self._get_rnn_output(input_bert_word, input_subword_idx, input_word, input_char, input_pos, mask=mask)
        # [batch, length_head, length_child]
        out_arc = self.biaffine(arc[0], arc[1], mask_query=mask, mask_key=mask)
        return out_arc, type

    def loss(self, input_bert_word, input_subword_idx, input_word, input_char, input_pos, heads, types, mask=None):
        # out_arc shape [batch, length_head, length_child]
        out_arc, out_type  = self(input_bert_word, input_subword_idx, input_word, input_char, input_pos, mask=mask)
    ...
    def decode(self, bert_words, sub_word_idx, input_word, input_char, input_pos, mask=None, leading_symbolic=0):
        """
        Args:
            input_word: Tensor
        ...
        """
        # out_arc shape [batch, length_h, length_c]
        out_arc, out_type = self(bert_words, sub_word_idx, input_word, input_char, input_pos, mask=mask)
